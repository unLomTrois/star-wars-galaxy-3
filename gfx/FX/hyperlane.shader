...
VertexShader =
{
	...
	[[
		VS_OUTPUT main(const VS_INPUT v )
		{
			...
			Out.vPrimaryColor = float3(0.5, 1, 1);
...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float fAlpha = clamp (0.23f - abs( v.vPos.x / 4700.f) - abs( v.vPos.y / 4700.f), 0.05f, 0.23f); // 'clamp' sets the min value to 0.06 and max to 0.21,'abs' gets rid of the minus,'info' command shows coordinates in the game.
